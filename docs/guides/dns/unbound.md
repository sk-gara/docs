...
**Important**: To be certain to have an up-to-date `root.hints` file, you should download it like described below. The maintainer of the `dns-root-data` package does not update it frequently. If in doubt, check the version of the provided `root.hints` file, it's stated at the top.
